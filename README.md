...
| `telegram_bot_admin_ids` | Список Telegram ID администраторов | ✅ |
...
- **Наборы референсов** → `storage/users/{user_id}/sets/{set_id}/` (файлы не перемещаются после генерации)
- **Использованные файлы** → `storage/users/{user_id}/used/` (при очистке `last_uploads` или удалении наборов)

### Кэширование файлов:

- Загруженные файлы кэшируются с сохранением Higgsfield URL
- Кэш действителен в течение `file_cache_ttl_days` дней (по умолчанию 7)
- При повторном использовании файла используется кэшированный URL вместо повторной загрузки
- Автоматическая очистка истекших записей кэша

### Наборы референсов:

- Пользователи могут создавать именованные наборы референсных изображений
- Файлы наборов хранятся в изолированных папках `sets/{set_id}/`
- Наборы можно использовать при генерации вместо загрузки новых фото
- Поддержка просмотра наборов в слайд-шоу, переименования и управления фото
...
- [x] Кэширование загруженных файлов с сохранением Higgsfield URL
- [x] Создание наборов референсов для лиц
- [x] Улучшение UX загрузки фото с интерактивными кнопками
- [x] Промпт-мастер (DeepSeek API)
